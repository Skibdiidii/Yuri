...
export function generateProfile(token: string | null = null, isMobile: boolean = false) {
  const seedVal = token
  ...
  let os, osVer, ua, superPropsObj;

  if (isMobile) {
    os = rng() > 0.5 ? "iOS" : "Android";
    if (os === "iOS") {
      osVer = "17.4";
      ua = `Discord-iOS/${build} CFNetwork/1494.0.7 Darwin/23.4.0`;
      superPropsObj = {
        os,
        browser: "Discord iOS",
        device: "iPhone13,2",
        system_locale: locale,
        client_version: "219.0",
        release_channel: "stable",
        device_vendor_id: crypto.randomUUID(),
        browser_user_agent: "",
        browser_version: "",
        os_version: osVer,
        client_build_number: 219000,
        client_event_source: null
      };
    } else {
      osVer = "34";
      ua = `Discord-Android/${build}`;
      superPropsObj = {
        os,
        browser: "Discord Android",
        device: "SM-G998B",
        system_locale: locale,
        client_version: "219.20",
        release_channel: "googleRelease",
        device_vendor_id: crypto.randomUUID(),
        browser_user_agent: "",
        browser_version: "",
        os_version: osVer,
        client_build_number: 219020,
        client_event_source: null
      };
    }
  } else {
    os = rng() > 0.2 ? "Windows" : "Mac OS X";
    osVer = pick(OS_VERSIONS[os as keyof typeof OS_VERSIONS]);
    ua =
      os === "Windows"
...
const _mobileProfileCache = new Map();
export function getProfile(token: string, isMobile: boolean = false) {
  const cache = isMobile ? _mobileProfileCache : _profileCache;
  if (!cache.has(token)) cache.set(token, generateProfile(token, isMobile));
  return cache.get(token);
}
export function rotateProfile(token: string, isMobile: boolean = false) {
  const cache = isMobile ? _mobileProfileCache : _profileCache;
  const p = generateProfile(token, isMobile);
  cache.set(token, p);
  return p;
}

export function headers(token: string | null = null, extra: any = {}, isMobile: boolean = false) {
  const p = token ? getProfile(token, isMobile) : generateProfile(token, isMobile);
  const h: any = {
